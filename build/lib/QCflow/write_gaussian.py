...
def write_gaussian(job_name, mol_name, smile, functional='B3LYP', basis_set='6-31G*', mol=None, 
                                                                                       torsion=None, conformer=None, old_chk=None):
    """
    ...
    if (job_name=='opt'):
        ...
        if old_chk is not None:
            old_chk = f'%OldChk={mol_name}_{job_name}.chk' # needed if starting from previous job
    ...
    if (job_name=='tor'):
        ...
        symbols = [a.GetSymbol() for a in mol.GetAtoms()]
        # get x y z coords
        #geometry=rdkit_predict_conf(smile)
    ...
    if (job_name=='pop_opt_n'):
        ...
        if old_chk is not None:
            ...
            calculation = 'opt, Pop=Full, Geom=Checkpoint'
        else:
            old_chk = ' '
            # get atom names
            symbols = [a.GetSymbol() for a in mol.GetAtoms()]
    ...
    if (job_name=='sp_hirsh'):

        torsion_data = f' \n'
        if old_chk is None:
            old_chk = f'%OldChk={mol_name}_opt.chk'
        calculation = 'SP, Geom=Checkpoint, Pop=Hirshfeld'
        mult_chg = '0 1'
        #reads the geometry information from the checkpoint file
